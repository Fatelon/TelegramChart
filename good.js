...
class TelegramChart {
    ...
    constructor(data, title, mode) {
        this.init(data, title, mode);
    }

    init(data, chartTitle, mode) {
    ...
    drawYScaleLabelsText(interval, rate) {
        const height = this.canvas.height;
        this.canvasContext.transform(1, 0, 0, -1, 0, height);
        // this.cx.fillStyle = 'rgba(150, 162, 170, ' + rate + ')';
...
xhr.onreadystatechange = (e) => {
    if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
        ...
        jsonData.forEach((data, i) => charts.push(new TelegramChart(data, 'Followers #' + (i + 1), mode)));
    }
};
xhr.send();
